...
CarrierWave.configure do |config|
  ...
  config.fog_credentials = {
      provider:              'AWS',                        # required
      aws_access_key_id:     'AKIAIAKDM6RWNFMPWEQA',                        # required
      aws_secret_access_key: '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',                        # required
      region:                'ap-southeast-1'                  # optional, defaults to 'us-east-1'
      # host:                  's3.example.com',             # optional, defaults to nil
      # endpoint:              'milephoto.s3-website-ap-southeast-1.amazonaws.com' # optional, defaults to nil
  }
  config.fog_directory  = 'cunchutong'                          # required
  config.fog_public     = true                                        # optional, defaults to true
  # config.fog_attributes = { 'Cache-Control' => "max-age=#{365.day.to_i}" } # optional, defaults to {}
end
